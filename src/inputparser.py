...
def ReadInputFilePath():
    print(
        "Please provide the absolute path to the CSV file with the relation"
        " you want to normalize:"
    )

    filePathInput = rf"{input()}"
...
def ReadFunctionalDependencies(attributes):
    print(
        "Please provide the functional dependencies of the relation (type "
        '"exit" when finished):'
    )
...
def ReadNormalForm():
    print(
        "Please provide the normal form you want to the relation to be in "
...
def ReadKeys(attributes):
    print("Please provide all of the candidate keys (type " '"exit" when finished):')
